    ...
    || {
        ...
        map.insert(
            ...
            PlatformExtensionDef {
                ...
                unprefixed_tools: false,
                hidden: false,
                client_factory: |ctx| Box::new(watcher::WatcherClient::new(ctx).unwrap()),
            },
        );

        map.insert(
            git_aware::EXTENSION_NAME,
            PlatformExtensionDef {
                ...
                unprefixed_tools: false,
                hidden: false,
                client_factory: |ctx| Box::new(git_aware::GitAwareClient::new(ctx).unwrap()),
            },
        );

        map.insert(
            gangus_memory::EXTENSION_NAME,
            PlatformExtensionDef {
                name: gangus_memory::EXTENSION_NAME,
                display_name: "Gangus Memory",
                description: "Injects GANGUS.md project memory into every turn",
                default_enabled: true,
                unprefixed_tools: false,
                hidden: false,
        ...
        map.insert(
            hooks::EXTENSION_NAME,
            PlatformExtensionDef {
                name: hooks::EXTENSION_NAME,
                display_name: "Hooks",
                description: "Pre/post tool hooks from .gangus/hooks.toml — shows active hook count every turn",
                default_enabled: false,
                unprefixed_tools: false,
                hidden: false,
                client_factory: |ctx| Box::new(hooks::HooksClient::new(ctx).unwrap()),
            },
        );

        map.insert(
            planner::EXTENSION_NAME,
            PlatformExtensionDef {
                ...
                unprefixed_tools: false,
                hidden: false,
                client_factory: |ctx| Box::new(planner::PlannerClient::new(ctx).unwrap()),
            },
        );

        map.insert(
            codeact::EXTENSION_NAME,
            PlatformExtensionDef {
                ...
                unprefixed_tools: false,
                hidden: false,
                client_factory: |ctx| Box::new(codeact::CodeActExtension::new(ctx).unwrap()),
            },
        );

        map.insert(
            context7::EXTENSION_NAME,
            PlatformExtensionDef {
                name: context7::EXTENSION_NAME,
                display_name: "Context7 Docs",
                description:
                    "Injects up-to-date library/framework documentation into MOIM based on conversation context",
                default_enabled: true,
                unprefixed_tools: false,
                hidden: false,
                client_factory: |ctx| Box::new(context7::Context7Client::new(ctx).unwrap()),
            },
        );
